...
// Function to check if the current telegramLine is the first or the last line of the telegram
bool checkLine() {
...
void loop() {
  while (p1Port.available()) {                            // Check the connection with the P1 port
    ...
    // If this is the last line, publish the values to the MQTT broker  
    if (checkLine()) {
      publishTelegram();
    }
  }
